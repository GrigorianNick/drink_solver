...
mod test {
    ...
    use crate::{ingredient_store::{IngredientSelector, IngredientStore}, store::Store};
    ...
    fn test_selector_quality() {
        ...
        for qual in vec![Quality::Low, Quality::Medium, Quality::High] {
            let selector = IngredientSelector{ name: None, quality: Some(qual), tags: None, in_stock: None };
            let result = store.select(&selector);
            assert_eq!(result.len(), 2);
        }

        // Quality::Any or None should fetch everything
        let selector = IngredientSelector{ name: None, quality: Some(Quality::Any), tags: None, in_stock: None };
        let result = store.select(&selector);
    ...
    fn test_selector_name() {
        ...
        let dupe_selector = IngredientSelector{ name: Some("dupe".into()), quality: None, tags: None, in_stock: None};
        let dupe_result = store.select(&dupe_selector);
        assert_eq!(dupe_result.len(), 2);
        let novel_selector = IngredientSelector{ name: Some("novel".into()), quality: None, tags: None, in_stock: None};
        let novel_result = store.select(&novel_selector);
        assert_eq!(novel_result.len(), 1);

        // Check that None gets everything
        let none_selector = IngredientSelector{ name: None, quality: None, tags: None, in_stock: None};
        let none_result = store.select(&none_selector);
        assert_eq!(none_result.len(), 3);

        // Check a bad name gets nothing
        let bad_selector = IngredientSelector{ name: Some("fake".into()), quality: None, tags: None, in_stock: None};
